...
class Debugger:
    ...
    def pointer_scan(self, address_list):
        pointers = []

        for addr in address_list:
            search = addr.to_bytes(8, 'little')
                ...
                if mbi.State == 0x1000:
                    if mbi.Protect in (0x04, 0x20):
                        try:
                            ...
                            offset = buffer.find(search)
                            if offset != -1:
                                pointer_addr = mbi.BaseAddress + offset
                                pointers.append(pointer_addr)
                                print("PTR ->", hex(pointer_addr))
                        except:
                            pass
...
if __name__ == "__main__":
    ...
    if dbg.attach(pid):
        ...
        results = dbg.scan_memory(value)
        dbg.pointer_scan(results)